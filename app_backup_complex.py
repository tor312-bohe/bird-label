...
def recognize_bird_hholove(image_path, location=None):
    ...
    try:
        ...
        if file_size > 2 * 1024 * 1024:  # 2MB
            raise ValueError(f"File size {file_size/1024/1024:.1f}MB exceeds 2MB limit")
...
def add_caption(img_path, cn, la, position="bottom_right"):
    ...
    # Target twice as big - 6% of the original image width for the Chinese text
    target_ratio = 0.06  # 6% of original image width (doubled from 3%)
    target_width = int(W * target_ratio)
    
    # Start with a larger initial size
    font_cn_size = max(int(W * 0.016), 16)  # Start at 1.6% of width, minimum 16px
    font_la_size = int(font_cn_size * 0.8)  # Latin slightly smaller
    
    print(f"🔍 Debug: Image size {W}x{H}, target width: {target_width}px (6% of {W}px = {target_ratio*100}%)")
    ...
    if font_cn and font_la:
        for iteration in range(50):  # More iterations for precision
            ...
            if font_cn_size >= min(W * 0.12, H * 0.08):  # Increased limits for larger fonts - 12% of width max
    ...
    line_spacing = max(18, int(h_cn * 0.28))
